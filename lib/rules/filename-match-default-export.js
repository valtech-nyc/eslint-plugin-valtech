...
module.exports = {
    ...
    create(context) {
        ...
        const getModuleName = declaration => {
            if (declaration.type === 'CallExpression' && declaration.callee.type === 'CallExpression') {
                return getModuleName(declaration.arguments[0]);
            }
            return declaration.name || (declaration.id && declaration.id.name);
        };
